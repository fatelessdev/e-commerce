...
export function ComboClient({ id, initialCombo }: { id: string; initialCombo?: Combo }) {
  ...
  // Extract variants arrays to local variables before useMemo to avoid optional chaining in dependency array
  ...
  const selectedStockA = selectedSizeA
    ? (stockMapA.get(`${selectedSizeA}|${requiredColorA ? selectedColorA : null}`) ?? 0)
    : null
  const selectedStockB = selectedSizeB
    ? (stockMapB.get(`${selectedSizeB}|${requiredColorB ? selectedColorB : null}`) ?? 0)
    : null
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {combo.productA.colors.map((color) => {
                      const available = selectedSizeA ? (stockMapA.get(`${selectedSizeA}|${color.name}`) ?? 0) > 0 : false
                      return (
                        <button
                          key={color.name}
                          type="button"
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {combo.productB.colors.map((color) => {
                      const available = selectedSizeB ? (stockMapB.get(`${selectedSizeB}|${color.name}`) ?? 0) > 0 : false
                      return (
                        <button
                          key={color.name}
                          type="button"
